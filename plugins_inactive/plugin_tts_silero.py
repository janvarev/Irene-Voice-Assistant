...
def init(core: VACore):
    ...
    if not os.path.isfile(local_file):
        torch.hub.download_url_to_file(
            "https://models.silero.ai/models/tts/ru/{}.pt".format(options["sileroId"]),
            local_file,
        )

    core.model = torch.package.PackageImporter(local_file).load_pickle(
        "tts_models", "model"
    )
...
def say(core: VACore, text_to_speech: str):
    # simple way
    paths = core.model.save_wav(texts=[text_to_speech], sample_rate=16000)
    ...
    for sentence in sentences:
        if sentence != "":
            paths = core.model.save_wav(texts=[sentence + "."], sample_rate=16000)
            # print(text_to_speech)
            # print(paths)
            for file in paths:
                core.play_wav(file)

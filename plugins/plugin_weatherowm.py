...
def start_with_options(core: VACore, manifest: dict):
    ...
    if (
        options["lat"] == 0 and options["lon"] == 0
    ):  # если не указаны координаты - получаем их
        ...
        try:
            ...
            res_str = requests.get(
                ...
                params={
                    "q": options["city"] + "," + options["country"],
            ...
            print(
                "OWM: city {0} located at lat={1}, lon={2}".format(
                    options["city"], lat, lon
                )
            )
        except:
            import traceback
    ...
    try:
        ...
        if addparam == "" and (phrase == "" or phrase == "сейчас"):
            ...
            text = "Сейчас {0}, ощущается как {1}. Влажность {2}, ветер {3} в секунду".format(
                num2text(int(temp)),
                num2text(int(feels_like)),
                vl_str,
                veter_str,
                data_one_call["daily"][0]["weather"][0]["description"],
            )
            print(text)
        ...
        if phrase == "завтра":
            ...
            text = "Завтра {0}, будет ощущаться как {1}. {3}. Ночью {2}".format(
                num2text(int(forecast_data["temp"]["day"])),
                num2text(int(forecast_data["feels_like"]["day"])),
                int(forecast_data["temp"]["night"]),
        ...
        if phrase == "послезавтра":
            ...
            text = "Послезавтра {0}, будет ощущаться как {1}. {3}. Ночью {2}".format(
                int(forecast_data["temp"]["day"]),
                int(forecast_data["feels_like"]["day"]),
                int(forecast_data["temp"]["night"]),
                forecast_data["weather"][0]["description"],
            )
            print(text)
        ...
        if addparam == "prognoz" or (addparam == "" and phrase == "прогноз"):
            ...
            text += "Завтра {3}, {0}. ".format(
                num2text(int(forecast_data["temp"]["day"])),
                num2text(int(forecast_data["feels_like"]["day"])),
                num2text(int(forecast_data["temp"]["night"])),
                forecast_data["weather"][0]["description"],
            )

...
### Версия 13.0 - VOSK streaming STT 0.56, распознавание уровня Whisper 

В версии 13.0 добавлена опция запуска с моделью vosk-stt-0.56
Эта модель значительно новее (сентябрь 2025 года), и по [данным бенчмарка](https://alphacephei.com/nsh/2025/04/18/russian-models.html)
в общем-то даже лучше Whisper - но при этом работает
- в стриминговом режиме (минимальная задержка)
- у вас на компьютере локально (а не в сети)

Для сравнения - процент ошибок распознавания (WER) на бенчмарке Sova devices (меньше - лучше)
- vosk-small-0.22 (старая модель) - 29.89 [источник](https://alphacephei.com/vosk/models)
- vosk-0.54 (у нас 0.56, вероятно, чуть лучше) - **11.6** [источник](https://alphacephei.com/nsh/2025/04/18/russian-models.html)
- Whisper Large V3 - 15.9 [источник](https://alphacephei.com/nsh/2025/04/18/russian-models.html)
...
- После чего запустите runva_vosk_sherpa.py

Способы уменьшения потребления ресурсов:
- Модель по умолчанию полная (https://huggingface.co/alphacep/vosk-model-streaming-ru).
- Вы можете там же скачать квантизацию до int8 (8 бит на вес нейросети вместо 32) - это ухудшит качество, но уменьшит потребление памяти
- Или вы можете скачать более маленькую модель отсюда: https://huggingface.co/alphacep/vosk-model-small-streaming-ru

Пока в бета-версии: 
- работает только с одним (первым) микрофоном на локальном девайсе
- вариант "по сети" не поддерживаются

Желающие могут доработать для работы в сети - сделать клиенты и прочее.
...
## Speech-to-Text через GigaSTT (GigaAM v3)

[GigaSTT](https://github.com/ekhodzitsky/gigastt) — локальный сервер распознавания русской
речи на модели GigaAM v3. Работает полностью на устройстве (CPU), без облака.
...
Дополнительные параметры: `-d` — выбор микрофона, `-r` — частота дискретизации,
`-l` — список аудиоустройств, `--debug` — вывод промежуточных результатов распознавания.

**Обращаем внимание, что разработчики Ирины не имеют отношения к данному распознавателю GigaSTT; используйте на свой страх и риск.**
